...
# grabs a random word from the list and returns it
def getWord():    
...
#initiates game
def playGame():
	#decides on the word the user needs to guess
    global word
    word = getWord()
    #resets variables
    global guessed_letters
    ...
    hangLabel['text'] = HANGMANPICS[0]
    inputGuess.delete(0,tk.END)

    
def guessButton(guess):
    ...
    global guessed_letters
    global word
    #check to see if ther are still tries left (this is redundant)
    if display == word:
        print("hi")
    # If tries left, attmpts the guess made by user    
    else:
        if guess in guessed_letters and len(guess) == 1:
	    	hangLabel['text'] = HANGMANPICS[hang] + "\n" + "\n" + "Fool, you have already guessed this:  " + guess
	    	...
	    	playLabel['text'] = display
	    	if display == word:
	    	    hangLabel['text'] = "You win.. T_T" + "\n" + HANGMANPICS[hang] + "\n" + "\n" +"click Play Game if you think youre smart enough"
        else:
	        return
	inputGuess.delete(0,tk.END)
...
titleLabel = tk.Label(canvas)
titleLabel.place(relx = 0.2, rely = 0, relwidth =0.6, relheight =0.1)
titleLabel['text'] = "Hangman"

playLabel = tk.Label(canvas, bg = "white")
playLabel.place(relx = 0.2, rely = 0.1, relwidth = 0.6, relheight = 0.1)

hangLabel = tk.Label(canvas, bg = "white")
hangLabel.place(relx = 0.2, rely = 0.25, relwidth = 0.6, relheight = 0.5)

inputGuess = tk.Entry(canvas)
inputGuess.place(relx = 0.2, rely=0.8, relwidth = 0.3, relheight = 0.08)

guessBut = tk.Button(canvas, text = "Guess", command = lambda: guessButton(inputGuess.get().upper()))
guessBut.place(relx = 0.5, rely = 0.8, relwidth = 0.3, relheight = 0.08)

playBut = tk.Button(canvas, text = "Play Game", command = playGame)
playBut.place(relx = 0.2, rely = 0.9, relwidth = 0.6, relheight = 0.08)
